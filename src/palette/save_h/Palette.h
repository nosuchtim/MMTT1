...
public:
	...
	void Set(std::string nm, std::string val) {
		...
		else if ( nm == "minmove" ) { minmove = string2double(val); }
	...
	void Increment(std::string nm, double amount) {
		...
		else if ( nm == "minmove" ) { minmove = adjust(minmove,amount,0.0,0.3); }
	...
	void Toggle(std::string nm) {
		...
		else if ( nm == "minmove" ) { NosuchDebug("minmove can't be toggled"); }
	...
	std::string Get(std::string nm) {
		...
		else if ( nm == "minmove" ) { return DoubleString(minmove); }
