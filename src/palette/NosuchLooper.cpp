...
NosuchLooper::NosuchLooper(PaletteHost* b) {
	NosuchDebug(2,"NosuchLooper constructor");
	_last_click = 0;
	_paletteHost = b;
...
NosuchLooper::~NosuchLooper() {
	NosuchDebug(2,"NosuchLooper destructor");
}
...
void
NosuchLooper::AdvanceClickTo(int click, NosuchScheduler* sched) {
	Lock();
	int nclicks = click - _last_click;
	static int last_printed_click = 0;
	if ( click >= (last_printed_click+10*NosuchScheduler::ClicksPerSecond) ) {
		NosuchDebug(1,"NosuchLooper::AdvanceClickTo click=%d now=%d",click,Pt_Time());
		last_printed_click = click;
	}
	// NosuchDebug("AdvanceClickTo click=%d last_click=%d nclicks=%d _loops.size=%d",
	// 	click,_last_click,nclicks,_loops.size());
	while ( nclicks-- > 0 ) {
		std::vector<NosuchLoop*>::iterator it = _loops.begin();
		for(; it != _loops.end(); it++) {
			NosuchLoop* lp = *it;
...
NosuchLoop::SendMidiLoopOutput(MidiMsg* mm) {
	_paletteHost->scheduler()->SendMidiMsg(mm,id());
	// OutputNotificationMidiMsg(mm,XXX);
}
...
isMatchingOff(SchedEvent* ev, MidiNoteOn* note) {
	MidiMsg* m = ev->midimsg;
	if ( m->MidiType() == MIDI_NOTE_OFF
		&& m->Channel() == note->Channel()
		&& m->Pitch() == note->Pitch() ) {
		// NosuchDebug("FOUND MATCHING NOTEOFF!");
		return true;
	}
	return false;
}
...
void NosuchLoop::AdvanceClickBy1() {
	Lock();
	// NosuchDebug("NosuchLoop::AdvanceClickBy1, loop id=%d click=%d _events.size=%d",_id,_click,_events.size());
	_click++;
	...
	for(; it != _events.end(); ) {
		...
		if ( ev->eventtype() == SchedEvent::CURSORMOTION ) {
			it++;
			NosuchDebug("LOOP CURSORMOTION!  event=%s",ev->DebugString().c_str());
			continue;
		}
		...
		case MIDI_NOTE_ON: {
			...
			if ( noteon->Velocity() < MIN_LOOP_VELOCITY ) {
				...
				NosuchDebug(1,"Note Velocity is < 5, should be removing");
				it = _events.erase(it);
				incrementit = false;
				...
				if ( offit != _events.end() ) {
					...
					MidiNoteOff* noteoff = (MidiNoteOff*)(evoff->midimsg);
					...
					NosuchDebug(1,"Another NEW DELETE of evoff");
					delete evoff;
				}
				NosuchDebug(1,"After removing from lp=%d, nnotes=%d",id(),NumNotes());

				NosuchDebug(1,"Another NEW DELETE of ev");
				delete ev;
				...
				if ( nnotes == 0 ) {
					NosuchDebug("No more notes in loop, CLEARING IT!");
					ClearNoLock();
				...
				NosuchDebug(1,"Fading velocity, vel=%d fade=%.4f",(int)(noteon->Velocity()),fade);
		...
		default:
			NosuchDebug("AdvanceClickBy1 can't handle miditype=%d",m->MidiType());
			break;
		}
		if ( incrementit ) {
			it++;
...
NosuchLoop::oldestNoteOn() {
	...
	for( ; it != _events.end(); it++ ) {
		SchedEvent* ev = *it;
		NosuchAssert(ev);
		if ( ev->eventtype() != SchedEvent::MIDI ) {
			continue;
		}
		MidiMsg* mm = ev->midimsg;
		NosuchAssert(mm);
...
NosuchLoop::removeNote(SchedEventIterator it) {
	...
	if ( ev->midimsg->MidiType() != MIDI_NOTE_ON ) {
		throw NosuchException("removeNote didn't get a noteon");
	}
	MidiNoteOn* noteon = (MidiNoteOn*)ev->midimsg;
	NosuchAssert(noteon);
...
NosuchLoop::removeOldestNoteOn() {
	...
	if ( it == _events.end() ) {
		NosuchDebug("Hmmm, removeOldestNoteOn didn't find an oldest note!?");
		Unlock();
		return;
	}
	MidiNoteOn* noteon = (MidiNoteOn*)((*it)->midimsg);
	NosuchAssert(noteon);
	it = _events.erase(it);
	// NosuchDebug(1,"Removing oldest noteon = %s",noteon->DebugString().c_str());
	SchedEventIterator offit = findNoteOff(noteon,it);
	delete noteon;
	if ( offit == _events.end() ) {
		NosuchDebug("Hmmm, removeOldestNoteOn didn't find noteoff for oldest note!?");
	} else {
		SchedEvent* ev = *offit;
		MidiNoteOff* noteoff = (MidiNoteOff*)(ev->midimsg);
		// NosuchDebug(1,"Removing oldest noteoff = %s",noteoff->DebugString().c_str());
		NosuchAssert(noteoff);
		_events.erase(offit);
		delete noteoff;

		NosuchDebug(1,"NEW DELETE CODE IN removeOldestNoteOn");
		delete ev;
	}
	Unlock();
}
...
NosuchLoop::ClearNoLock() {
	while ( true ) {
		...
		case SchedEvent::CURSORMOTION: {
			NosuchCursorMotion* cm = ev->_cursormotion;
			NosuchDebug("Clearing Cursormotion event!");
			delete cm;
			break;
			}
		}
