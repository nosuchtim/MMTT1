...
/*
NOTE: we forcibly use all the ANSI versions of the macros, typedefs,
and functions (like MIDIINCAPSA, MIDIOUTCAPSA, LPMIDIINCAPSA,
LPMIDIOUTCAPSA, midiInGetDevCapsA, midiOutGetDevCapsA) so that we can
consistently use ANSI (char*) strings, even when compiling with
UNICODE defined (where wchar_t* strings are normally expected).
There are only a couple of places (like the device names) where strings
are exposed in the external interface of portmidi, so this isn't that
much of a problem even when portmidi is compiled and used from within
a UNICODE environment.
*/

/* global winmm device info */
MIDIINCAPSA *midi_in_caps = NULL;
MIDIINCAPSA midi_in_mapper_caps;
UINT midi_num_inputs = 0;
MIDIOUTCAPSA *midi_out_caps = NULL;
MIDIOUTCAPSA midi_out_mapper_caps;
